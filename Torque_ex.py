...
class MainReacher():
    ...
    def phi(self, x, ee_pos):
        diffX = x - ee_pos
        dist = math.sqrt(diffX[0]*diffX[0]+diffX[1]*diffX[1]+diffX[2]*diffX[2])
        gradX = diffX/dist
        if dist <= 2:
            return np.matrix(np.array(((1/dist)- (1/2))*(1/(dist*dist)) * gradX).reshape(3, 1))            
        else:
            return np.matrix(np.array([0, 0, 0]).reshape(3, 1))
    ...
    def go(self):
        ...
        for i in range(1000000):
            ...
            torques = J.T*ee_desired_force - grav_opposite_torques - J.T * self.phi([2.2,2, 0], ee_pos)
