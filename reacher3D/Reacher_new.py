...
class ReacherEnv(gym.Env):
    ...
    def rand_target(self):
        ...
        #new stuff
        if self.counter < 7:
            posses = [np.array([1.7, -1, 0.3]).reshape(3, 1), np.array([0.3, 1.9, -0.3]).reshape(3, 1), np.array([-1.2, 1.5, 1.6]).reshape(3, 1), np.array([-0.5, -1, -0.3]).reshape(3, 1), np.array([0.2, -1.4, 0.3]).reshape(3, 1), np.array([1.8, -1.1, -1.3]).reshape(3, 1), np.array([-0.7, 0.4, -1.4]).reshape(3, 1)]
            pos = posses[self.counter]
            self.counter += 1
    ...
    def _render(self, mode='human', close=False):
        ...
        if self.viewer is None:
            ...
            '''
            #new stuff
            ob = rendering.make_sphere(0.1)
            ob.set_color(0, 0, 0)
            self.colours.append([0,0,0])
            self.ob_transform1 = rendering.Transform()
            self.ob_transform1.set_translation(2.2,2, 0)
            ob.add_attr(self.ob_transform1)
            self.viewer.add_geom(ob)
            
            fixtarget = rendering.make_sphere(0.1)
            fixtarget.set_color(200, 200, 0)
            self.colours.append([200,200,0])
            self.fix_transform1 = rendering.Transform()
            self.fix_transform1.set_translation(2,2.4, 0)
            fixtarget.add_attr(self.fix_transform1)
            self.viewer.add_geom(fixtarget)
