...
class ReacherEnv(gym.Env):
    ...
    def __init__(self):
        ...
        if self.use_new_reacher:
            ...
            self.ob_rad = []
            self.noObjs = 5
            self.obj_radius_base = 0.35
            self.obj_radius_max_var = 0.05
            self.obj_boxes = []
            for x in range(-2,3):
                for y in range(-2,3):
                    for z in range(-2,3):
                        if not(x>=0 and ((y>=-1 and y <= 1)or(z>=-1 and z <= 1))):
                            self.obj_boxes.append(np.array([x,y,z]).astype(np.float64))
            self.ob_pos_range = 6
        #------END NEW CODE--------
    ...
    def rand_target(self):
        #----- NEW CODE---------
        # Needed to ensure the random valid and invalid targets are not impossible to reach (by preventing them being in obstacles)
        if self.use_new_reacher:
            pos = self.get_random_pos(is_obstacle=False).reshape((3,1))
            # Make sure target position is inbounds
            while abs(np.max(pos))>2:
                self.obj_boxes.append(pos.flatten())
                pos = self.get_random_pos(is_obstacle=False).reshape((3,1))
            self.invalpos = self.get_random_pos(is_obstacle=False).reshape((3,1))
    ...
    def _step(self,(jas,jvs,target_ja,torques)):
        ...
        if(self.targetTime>1 or self.targetTime2>1):
            #--------NEW CODE--------------
            if self.use_new_reacher:
                # Free the old target positions for potential positions for future targets
                self.obj_boxes.append(self.targetPos.flatten())
                self.obj_boxes.append(self.invalpos.flatten())
            #--------END NEW CODE----------
            self.targetPos=self.rand_target()
    ...
    def _render(self, mode='human', close=False):
        ...
        if self.viewer is None:
            ...
            if self.use_new_reacher:
                for i in range(0,self.noObjs):
                    self.ob_rad.append(random.random()*self.obj_radius_max_var+self.obj_radius_base)
                ...
                self.ground_truth_end_effector = None
            #----------END NEW CODE----------------
            ...
            #--------NEW CODE-------------------
            # This was necessary to allow detecting the black obstacles without confusing the rod as one too
            if self.use_new_reacher:
        ...
        #------START NEW STUFF---------------------------------------
        if self.use_new_reacher:
            if self.ground_truth_end_effector is not None:
                closest = 0
                closest_dist = float('inf')
                danger = False
                # Add the obstacles that the end effector is in the radius of influence of
                for i in range(0,len(self.ob_pos)):
                    ...
                    if dist <= r:
                        if self.flash:
                            objectIndi = rendering.make_sphere(0.1)
                    ...
                    if dist < closest_dist:
                        closest = i
                        closest_dist = dist
                # Add the cloesst obstacle to the end effector
                if not danger:
                    objectIndi = rendering.make_sphere(0.1)
                    objectIndi.set_color(255, 255, 255)
                    objectIndiTrans = rendering.Transform()
                    objectIndiTrans.set_translation(self.ob_pos[closest][0],self.ob_pos[closest][1],self.ob_pos[closest][2])
                    objectIndi.add_attr(objectIndiTrans)
                    self.viewer.add_onetime(objectIndi)
    ...
    #------START NEW STUFF---------------------------------------
    def get_random_pos(self, is_obstacle=True):
        max_rand = 0.09
        if is_obstacle:
            found_overlap = True
            while found_overlap:
                selected = random.randint(0,len(self.obj_boxes)-1)
                pos = self.obj_boxes[selected]
                found_overlap = False
                for i in range(0,len(self.ob_pos)):
                    other_pos = self.ob_pos[i]
                    # Ensure x-axis doing overlap to make lining up obstacles over both planes using image system possible
                    if abs(other_pos[0]-pos[0])<=max_rand:
                        found_overlap = True
                        break
            pos += (np.random.random_sample(size=(3,))-0.5)*max_rand
        else:
            found_overlap = True
            while found_overlap:
                selected = random.randint(0,len(self.obj_boxes)-1)
                ...
                found_overlap = False
                for i in range(0,len(self.ob_pos)):
                    diffPos = pos - self.ob_pos[i]
                    dist = np.sqrt(diffPos[0]*diffPos[0]+diffPos[1]*diffPos[1]+diffPos[2]*diffPos[2])
                    r = self.ob_rad[i]*8
                    # Ensure that the target is well outside the radius of influence so it is likely to be reachable
                    if dist <= r:
                        found_overlap = True
                        break
