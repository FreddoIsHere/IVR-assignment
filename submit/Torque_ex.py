...
class MainReacher():
    ...
    def detect_obstacles_on_plane(self, image):
        ...
        obstacles = []
        for k in keypoints:
            points = self.coordinate_convert(np.array([k.pt[0],k.pt[1]]))
            print("Points (%s,%s)"%(points[0],points[1]))
            # Divide by self.env.resolution to get radius in coordinate form
            radius = (k.size/2)/self.env.resolution
            obstacles.append([points[0],points[1],radius])
        print("-----")
    ...
    def detect_obstacles(self, xyarray, xzarray):
        ...
        obsts = []
        obst_x_dists = []
        x_shorters = np.array([k[0] for k in shorter])
        for i in range(0,len(longer)):
            l = longer[i]
            best_shorter = np.argmin(np.abs(x_shorters-l[0]))
            x = np.mean([shorter[best_shorter][0],l[0]])
            if len(xyobj)>len(xzobj):
                y = xyobj[i][1]
                z = xzobj[best_shorter][1]
            else:
                z = xzobj[i][1]
                y = xyobj[best_shorter][1]
            rad = np.max([shorter[best_shorter][2],l[2]])
            obsts.append((x,y,z,rad))
            best_dist = abs(shorter[best_shorter][0]-l[0])
            obst_x_dists.append(best_dist)
        return obsts
    ...
    def phi(self, xs, ee_pos):
        ...
        for x in xs:
            ...
            gradX = diffX/dist
            if dist <= x[3]+1:
                sumofForce += np.matrix(np.array(((1/dist)- (1/2))*(1/(dist*dist)) * gradX).reshape(3, 1))
                print("DODGED (%s,%s,%s)"%(x[0],x[1],x[2]))
                time.sleep(100)
            else:
                sumofForce += np.matrix(np.array([0.0, 0.0, 0.0]).reshape(3, 1))
                print("DIDN'T DODGE (%s,%s,%s)"%(x[0],x[1],x[2]))
        print("--------")
        return sumofForce
    ...
    def go(self):
        ...
        for i in range(1000000):
            ...
            #print(self.phi([2.2,2, 0], ee_pos))
            #print(ee_desired_force)

            obsts = self.detect_obstacles(arrxy,arrxz)

            torques = J.T*ee_desired_force - grav_opposite_torques - J.T * self.phi(obsts, ee_pos)
